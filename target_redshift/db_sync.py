...
def flatten_key(k, parent_key, sep):
    ...
    while len(sep.join(inflected_key)) >= 127 and reducer_index < len(inflected_key):
        ...
        inflected_key[reducer_index] = \
            (reduced_key if len(reduced_key) > 1 else inflected_key[reducer_index][
                                                      0:3]).lower()
        reducer_index += 1
    ...
    for k, v in d['properties'].items():
        ...
        if 'type' in v.keys():
            if 'object' in v['type'] and 'properties' in v and level < max_level:
                items.extend(
                    flatten_schema(v, parent_key + [k], sep=sep, level=level + 1,
...
def _should_json_dump_value(key, value, flatten_schema=None):
    ...
    if flatten_schema and key in flatten_schema and 'type' in flatten_schema[
        key] and set(flatten_schema[key]['type']) == {'null', 'object', 'array'}:
        return True
...
def flatten_record(d, flatten_schema=None, parent_key=[], sep='__', level=0,
                   max_level=0):
    items = []
    for k, v in d.items():
        new_key = flatten_key(k, parent_key, sep)
        if isinstance(v, collections.MutableMapping) and level < max_level:
            items.extend(flatten_record(v, flatten_schema, parent_key + [k], sep=sep,
                                        level=level + 1, max_level=max_level).items())
        else:
            items.append((new_key, json.dumps(v) if _should_json_dump_value(k, v,
                                                                            flatten_schema) else v))
    return dict(items)
...
class DbSync:
    def __init__(self, connection_config, stream_schema_message=None, table_cache=None):
        ...
        if self.intermediary_role:
            ...
            self.connection_config['aws_access_key_id'] = credentials[
                "aws_access_key_id"]
            self.connection_config['aws_secret_access_key'] = credentials[
                "aws_secret_access_key"]
            self.connection_config['aws_session_token'] = credentials[
                "aws_session_token"]
        ...
        if self.stream_schema_message is not None:
            ...
            stream_schema_name = stream_name_to_dict(stream_name)[
                'schema_name']  # ED NOTES maybe we can just change it here?
    ...
    def record_primary_key_string(self, record):
        ...
        flatten = flatten_record(record, self.flatten_schema,
                                 max_level=self.data_flattening_max_level)
        try:
            key_props = [str(flatten[p]) for p in
                         self.stream_schema_message['key_properties']]
        except Exception as exc:
            LOGGER.info("Cannot find {} primary key(s) in record: {}".format(
    ...
    def put_to_s3(self, file, stream, count, suffix=""):
        ...
        LOGGER.info(
            "Target S3 bucket: {}, local file: {}, S3 key: {}".format(bucket, file,
                                                                      s3_key))
        ...
        try:
            ...
            LOGGER.info("AWS Token expired, refreshing credentials and retrying "
                        "upload")
            self.s3 = boto3.client('s3', self.check_and_return_credentials(
    ...
    def load_csv(self, s3_key, count, size_bytes, compression=False):
        ...
        LOGGER.info("Loading {} rows into {}".format(count, self.table_name(stream,
                                                                            is_stage=True)))
        ...
        with self.open_connection() as connection:
            with connection.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                ...
                """.format(aws_role_arn=self.connection_config[
                    'aws_redshift_copy_role_arn']) if self.connection_config.get(
                ...
                """.format(
                    ...
                    aws_session_token="SESSION_TOKEN '{}'".format(
                        self.connection_config[
                            'aws_session_token']) if self.connection_config.get(
                ...
                LOGGER.debug("Running query: {}".format(copy_sql))
                    ...
                    if len(stream_schema_message['key_properties']) > 0:
                        ...
                        if not self.skip_updates:
                            ...
                            """.format(
                                target_table,
                                ', '.join(
                                    ['{} = s.{}'.format(c['name'], c['name']) for c in
                                     columns_with_trans]),
                                stage_table,
                                self.primary_key_merge_condition()
                            )
                            LOGGER.debug("Running query: {}".format(update_sql))
                        ...
                        """.format(
                            ...
                            self.primary_key_merge_condition(),
                            ' AND '.join(
                                ['{}.{} IS NULL'.format(target_table, c) for c in
                                 primary_column_names(stream_schema_message)])
                        )
                        LOGGER.debug("Running query: {}".format(insert_sql))
                ...
                try:
                    ...
                    if (
                        ...
                        LOGGER.error("Value too long for character type.")
                        LOGGER.error(
    ...
    def _increase_all_varchar_columns_to_max_size(self, table: str, schema: str):
        ...
        with self.open_connection() as connection:
            with connection.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                LOGGER.info(f"Running: {column_query}")
                ...
                if len(column_names) > 0:
                    ...
                    for column in column_names:
                        ...
                        try:
                            ...
                            LOGGER.info(f"Increased size for: {column}")
                    ...
                    LOGGER.info("All varchar columns are already at maximum size.")

    def primary_key_merge_condition(self):
        stream_schema_message = self.stream_schema_message
        names = primary_column_names(stream_schema_message)
        return ' AND '.join(['{}.{} = s.{}'.format(
            self.table_name(stream_schema_message['stream'], False), c, c) for c in
            names])
    ...
    def create_table_query(self, is_stage=False):
        ...
        primary_key = ["PRIMARY KEY ({})".format(
    ...
    def grant_usage_on_schema(self, schema_name, grantee, to_group=False):
        query = "GRANT USAGE ON SCHEMA {} TO {} {}".format(schema_name,
                                                           'GROUP' if to_group else '',
                                                           grantee)
        LOGGER.info(
            "Granting USAGE privilege on '{}' schema to '{}'... {}".format(schema_name,
                                                                           grantee,
                                                                           query))
        self.query(query)
    ...
    def create_schema_if_not_exists(self):
        ...
        if self.table_cache:
            schema_rows = list(
                filter(lambda x: x['table_schema'] == schema_name.lower(),
                       self.table_cache))
    ...
    def get_tables(self, table_schema=None):
        return self.query("""SELECT LOWER(table_schema) table_schema, LOWER(table_name) table_name
            ...
            "LOWER(table_schema)" if table_schema is None else "{}".format(
                table_schema.lower())
        ))
    ...
    def update_columns(self):
        ...
        if self.table_cache:
            columns = list(
                filter(lambda x: x['table_schema'] == self.schema_name.lower() and
        ...
        columns_to_replace = [
            ...
            if name.lower() in columns_dict and
               columns_dict[name.lower()]['data_type'].lower() != column_type(
                properties_schema, with_length=False).lower() and
    ...
    def sync_table(self):
        ...
        stream = stream_schema_message['stream']
        table_name = self.table_name(stream, is_stage=False, without_schema=True)
        table_name_with_schema = self.table_name(stream, is_stage=False,
                                                 without_schema=False)
        ...
        if self.table_cache:
            ...
            found_tables = [table for table in
                            (self.get_tables(self.schema_name.lower()))
